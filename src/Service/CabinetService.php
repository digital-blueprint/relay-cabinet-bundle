...
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\BinaryDocumentType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\Checksum;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\DualDeliveryRequest;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\ErrorType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\MetaData as DualDeliveryMetadata;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\PayloadAttributesType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\PayloadType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\ProcessingProfile;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\RecipientType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\SenderData;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDelivery\SenderType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryNotification\DualNotificationRequestType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryNotification\StatusRequestType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPersonData\CorporateBodyType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPersonData\DeliveryAddress;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPersonData\FamilyName;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPersonData\PersonDataType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPersonData\PersonNameType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPersonData\PhysicalPersonType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPersonData\PostalAddressType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPreAddressing\DualDeliveryPreAddressingRequestType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPreAddressing\DualDeliveryPreAddressingResponseType;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPreAddressing\MetaData as PreMetaData;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPreAddressing\Recipient;
use Dbp\Relay\CabinetBundle\DualDeliveryApi\Types\DualDeliveryPreAddressing\Recipients;
use Dbp\Relay\CabinetBundle\DualDeliveryProvider\Vendo\DeliveryQuality;
use Dbp\Relay\CabinetBundle\DualDeliveryProvider\Vendo\ProcessingProfile as VendoProcessingProfile;
use Dbp\Relay\CabinetBundle\DualDeliveryProvider\Vendo\Vendo;
use Dbp\Relay\CabinetBundle\Entity\DeliveryStatusChange;
use Dbp\Relay\CabinetBundle\Entity\PreAddressingRequest;
use Dbp\Relay\CabinetBundle\Entity\Request;
use Dbp\Relay\CabinetBundle\Entity\RequestFile;
use Dbp\Relay\CabinetBundle\Entity\RequestRecipient;
...
class CabinetService implements LoggerAwareInterface
{
    ...
    public function getRequestById(string $identifier): Request
    {
        ...
        if (!$request) {
            throw ApiError::withDetails(Response::HTTP_NOT_FOUND, 'Request was not found!', 'cabinet:request-not-found');
        }
    ...
    public function getDeliveryStatusChangeById(string $identifier): ?DeliveryStatusChange
    {
        ...
        if (!$deliveryStatusChange) {
            throw ApiError::withDetails(Response::HTTP_NOT_FOUND, 'DeliveryStatusChange was not found!', 'cabinet:request-status-change-not-found');
        }
        ...
        if ($deliveryStatusChange->getFileStorageSystem() === self::FILE_STORAGE_BLOB) {
            ...
            if (strlen($blobIdentifier) > 50) {
                throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'DeliveryStatusChange has invalid blob identifier!', 'cabinet:delivery-status-change-blob-identifier-invalid');
            }
    ...
    public function getRequestRecipientById(string $identifier): RequestRecipient
    {
        ...
        if (!$requestRecipient) {
            throw ApiError::withDetails(Response::HTTP_NOT_FOUND, 'RequestRecipient was not found!', 'cabinet:request-recipient-not-found');
        }
    ...
    public function getRequestFileById(string $identifier): ?RequestFile
    {
        ...
        if (!$requestFile) {
            throw ApiError::withDetails(Response::HTTP_NOT_FOUND, 'RequestFile was not found!', 'cabinet:request-file-not-found');
        }
        ...
        if ($requestFile->getFileStorageSystem() === self::FILE_STORAGE_BLOB) {
            ...
            if (strlen($blobIdentifier) > 50) {
                throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'RequestFile has invalid blob identifier!', 'cabinet:request-file-blob-identifier-invalid');
            }
    ...
    public function getRequestFilesByRequestId(string $identifier): array
    {
        ...
        if (!$requestFiles) {
            throw ApiError::withDetails(Response::HTTP_NOT_FOUND, 'RequestFiles were not found!', 'cabinet:request-files-not-found');
        }
    ...
    public function updateRequest(Request $request): Request
    {
        try {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'Request could not be updated!', 'cabinet:request-not-created', ['message' => $e->getMessage()]);
        }
    ...
    public function createRequest(Request $request): Request
    {
        ...
        try {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'Request could not be created!', 'cabinet:request-not-created', ['message' => $e->getMessage()]);
        }
    ...
    public function handleRequestRecipientStorage(RequestRecipient $requestRecipient): RequestRecipient
    {
        ...
        if ($requestRecipient->getIdentifier() === '') {
            ...
            if ($requestRecipient->getCabinetRequest()->isSubmitted()) {
                throw ApiError::withDetails(Response::HTTP_BAD_REQUEST, 'Submitted requests cannot be modified!', 'cabinet:request-submitted-read-only');
            }
    ...
    public function createRequestRecipient(RequestRecipient $requestRecipient): RequestRecipient
    {
        ...
        $request = $this->getRequestById($requestRecipient->getCabinetRequestIdentifier());
        ...
        try {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'RequestRecipient could not be created!', 'cabinet:request-recipient-not-created', ['message' => $e->getMessage()]);
        }
    ...
    public function updateRequestRecipient(RequestRecipient $requestRecipient): RequestRecipient
    {
        try {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'RequestRecipient could not be updated!', 'cabinet:request-recipient-not-created', ['message' => $e->getMessage()]);
        }
    ...
    public function createRequestFile(File $uploadedFile, string $cabinetRequestIdentifier): RequestFile
    {
        $data = $uploadedFile->getContent();
        $requestFile = new RequestFile();

        // A request object needs to be set for the ORM, setting the identifier only will not persist it
        $requestFile->setCabinetRequestIdentifier($cabinetRequestIdentifier);
        $request = $this->getRequestById($requestFile->getCabinetRequestIdentifier());
        ...
        if ($this->fileStorage === self::FILE_STORAGE_BLOB) {
            $fileStorageIdentifier = $this->blobService->uploadRequestFile($cabinetRequestIdentifier, $fileName, $data);
        ...
        try {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'RequestFile could not be created!', 'cabinet:request-file-not-created', ['message' => $e->getMessage()]);
        }
    ...
    public function createDeliveryStatusChange(string $requestRecipientIdentifier, int $statusType, string $description = '', ?string $fileData = null): DeliveryStatusChange
    {
        ...
        $deliveryStatusChange->setCabinetRequestRecipientIdentifier($requestRecipientIdentifier);
        $requestRecipient = $this->getRequestRecipientById($deliveryStatusChange->getCabinetRequestRecipientIdentifier());
        ...
        if ($fileData) {
            ...
            if ($fileStorage === self::FILE_STORAGE_BLOB) {
                try {
                    $request = $requestRecipient->getCabinetRequest();
        ...
        try {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'DeliveryStatusChange could not be created!', 'cabinet:request-status-not-created', ['message' => $e->getMessage()]);
        }
    ...
    public function submitRequest(Request $request, bool $direct = false, bool $printRequestXml = false)
    {
        ...
        if ($direct) {
            ...
            $this->createAndCabinetRequestSubmissionMessage($request);
        }
    }
    ...
    public function createAndCabinetRequestSubmissionMessage(Request $request): RequestSubmissionMessage
    {
        $message = new RequestSubmissionMessage($request);
        $this->bus->cabinet($message);
    ...
    public function checkRequestReadyForSubmit(Request $request)
    {
        if ($request->isSubmitted()) {
            throw ApiError::withDetails(Response::HTTP_BAD_REQUEST, 'Request was already submitted!', 'cabinet:request-already-submitted');
        }

        $requestRecipients = $request->getRecipients();
        if ($requestRecipients->count() === 0) {
            throw ApiError::withDetails(Response::HTTP_BAD_REQUEST, 'Request has no recipients!', 'cabinet:request-has-no-recipients');
        }

        $gz = $request->getReferenceNumber();
        if (!Vendo::isValidGZForSubmission($gz)) {
            throw ApiError::withDetails(Response::HTTP_BAD_REQUEST, "referenceNumber wasn't set correctly!", 'cabinet:request-invalid-reference-number');
        }

        $name = $request->getName();
        if (trim($name) === '') {
            throw ApiError::withDetails(Response::HTTP_BAD_REQUEST, 'name must not be empty!', 'cabinet:request-name-empty');
        }

        if ($request->getFiles()->count() === 0) {
            throw ApiError::withDetails(Response::HTTP_BAD_REQUEST, 'Request has no files!', 'cabinet:request-has-no-files');
        }
        ...
        foreach ($requestRecipients as $requestRecipient) {
            if (!$requestRecipient->isPostalDeliverable() && !$requestRecipient->isElectronicallyDeliverable()) {
                throw ApiError::withDetails(Response::HTTP_BAD_REQUEST, 'Request has a recipient without a delivery method!', 'cabinet:request-has-recipient-without-delivery-method');
            }
        }
    }
    ...
    public function handleRequestSubmissionMessage(RequestSubmissionMessage $message)
    {
        ...
        // TODO: Cabinet another delayed message if Vendo request failed? (this isn't even possible now since DualDeliveryRequests are made for each recipient)
    ...
    public function doPreAddressingSoapRequest(string $givenName, string $familyName, \DateTimeInterface $birthDate): DualDeliveryPreAddressingResponseType
    {
        ...
        try {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'PreAddressing request failed!', 'cabinet:request-pre-addressing-failed', ['message' => $e->getMessage()]);
        }
        ...
        if ($response->getStatus()->getText() !== 'SUCCESS') {
            ...
            throw ApiError::withDetails(Response::HTTP_INTERNAL_SERVER_ERROR, 'PreAddressing request failed!', 'cabinet:request-pre-addressing-failed', ['message' => implode(', ', $errorTexts)]);
        }
    ...
    public function doPreAddressingSoapRequestForPreAddressingRequest(PreAddressingRequest &$preAddressingRequest)
    {
        ...
        if (count($addressingResults) === 0) {
            throw ApiError::withDetails(Response::HTTP_NOT_FOUND, 'Person was not found!', 'cabinet:request-pre-addressing-not-found', ['message' => 'No addressing results found!']);
        }
    ...
    public function doDualDeliveryStatusRequestSoapRequest(RequestRecipient $recipient): bool
    {
        ...
        try {
            ...
            throw ApiError::withDetails(
                ...
                'cabinet:status-request-soap-error',
                [
                    'recipient-id' => $recipient->getIdentifier(),
        ...
        if ($status === 0) {
            ...
            throw ApiError::withDetails(
                Response::HTTP_INTERNAL_SERVER_ERROR, 'StatusRequest request failed!', 'cabinet:status-request-failed', [
                    'recipient-id' => $recipient->getIdentifier(),
     ...
     * @param bool $printRequestXml If true print the request xml to the cli
     */
    public function doDualDeliveryRequestSoapRequest(Request &$cabinetRequest, bool $printRequestXml = false): bool
    {
        $service = $this->dd->getClient();
        $dualDeliveryPayloads = [];

        /** @var RequestFile[] $files */
        $files = $cabinetRequest->getFiles();

        // For some reasons files are not loaded by default
        if (count($files) === 0) {
            $files = $this->getRequestFilesByRequestId($cabinetRequest->getIdentifier());
            $this->logWarning('Request had no files and files were reloaded!', [
                'recipient-id' => $cabinetRequest->getIdentifier(),
            ]);
        }
        ...
        //
        $senderProfile = $this->dd->getSenderProfile();
        $senderFullName = trim($cabinetRequest->getSenderFullName() ?? '');
        $senderOrganizationName = trim($cabinetRequest->getSenderOrganizationName() ?? '');
        ...
        if ($senderFullName !== '') {
            ...
            $senderPostalAddress = new PostalAddressType(
                null,
                $cabinetRequest->getSenderPostalCode(),
                $cabinetRequest->getSenderAddressLocality(),
                new DeliveryAddress(
                    $cabinetRequest->getSenderStreetAddress(),
                    $cabinetRequest->getSenderBuildingNumber()
                )
            );

            $senderPostalAddress->setCountryCode($cabinetRequest->getSenderAddressCountry());
        ...
        $gz = $cabinetRequest->getReferenceNumber();

        /** @var RequestRecipient $recipient */
        foreach ($cabinetRequest->getRecipients() as $recipient) {
            ...
            try {
                ...
                throw ApiError::withDetails(
                    ...
                    'cabinet:dual-delivery-request-soap-error',
                    [
                        'request-id' => $cabinetRequest->getIdentifier(),
                        'recipient-id' => $recipient->getIdentifier(),
                        'message' => $e->getMessage(),
                    ]
                );
            ...
            if ($response->getStatus()->getText() !== 'SUCCESS') {
                ...
                throw ApiError::withDetails(
                    Response::HTTP_INTERNAL_SERVER_ERROR, 'DualDelivery request failed!', 'cabinet:dual-delivery-request-failed', [
                        'request-id' => $cabinetRequest->getIdentifier(),
                        'recipient-id' => $recipient->getIdentifier(),
                        'message' => $errorText,
                    ]
            ...
            //
            // A new entity was found through the relationship 'Dbp\\\\Relay\\\\CabinetBundle\\\\Entity\\\\RequestRecipient#request'
            // that was not configured to cascade persist operations for entity: Dbp\\\\Relay\\\\CabinetBundle\\\\Entity\\\\Request@658.
            ...
            // causes the problem implement 'Dbp\\\\Relay\\\\CabinetBundle\\\\Entity\\\\Request#__toString()' to get a clue.
            $queryBuilder = $this->em->createQueryBuilder();
            ...
            try {
                ...
                throw ApiError::withDetails(
                    ...
                    'cabinet:request-recipient-not-updated', [
                        'request-id' => $cabinetRequest->getIdentifier(),
                        'recipient-id' => $recipient->getIdentifier(),
    ...
    protected function setRecipientDeliveryEndDate(RequestRecipient $recipient)
    {
        ...
        try {
            ...
            throw ApiError::withDetails(
                ...
                'cabinet:request-recipient-delivery-end-date-not-set', [
                    'recipient-id' => $recipient->getIdentifier(),
                    'message' => $e->getMessage(),
                ]
            );
