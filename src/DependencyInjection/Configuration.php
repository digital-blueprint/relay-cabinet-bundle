            ...
            ->children()
                ...
                ->arrayNode('sync')
                   ->addDefaultsIfNotSet()
                    ->children()
                        ->booleanNode('enabled')
                            ->info('Whether automatic background synchronization is enabled')
                            ->defaultValue(false)
                        ->end()
                        ->scalarNode('schedule')
                ...
                ->arrayNode('typesense')
                    ->children()
                        ...
                        ->booleanNode('search_partitions_split_collection')
                            ...
                            ->defaultValue(false)
                        ->end()
                        ->integerNode('search_max_partition_pages')
                            ->info('Number of pages to query internally if partitioning is used')
                            ->defaultValue(4)
                        ->end()
